...
class MultipleGridAnchorGenerator(anchor_generator.AnchorGenerator):
  ...
  def _generate(self, feature_map_shape_list, im_height=1, im_width=1):
    ...
    #tf.logging.info('base_anchor_size:%0.2f', base_anchor_size[0])
    #tf.logging.info('base_anchor_size:%0.2f', base_anchor_size[1])
   
    #sess = tf.Session()
    # Evaluate the tensor `c`.
    #print(sess.run(base_anchor_size[0]))
    #print(sess.run(base_anchor_size[1]))
